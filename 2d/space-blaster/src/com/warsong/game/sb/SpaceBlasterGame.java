...
import com.warsong.game.sb.common.AudioClip;
import com.warsong.game.sb.common.Tools;
...
public class SpaceBlasterGame extends ArcadeGame {
	...
	public void initialize() {
		...
		ids = new int[] { R.drawable.sb_boom0, R.drawable.sb_boom1, R.drawable.sb_boom2, R.drawable.sb_boom3,
				R.drawable.sb_boom4 };
	...
	public boolean keyUp(int key) { // KeyEvent e,
		if (key == KeyEvent.KEYCODE_DPAD_LEFT || key == KeyEvent.KEYCODE_DPAD_RIGHT || key == KeyEvent.KEYCODE_Q
				|| key == KeyEvent.KEYCODE_W)
			dx = 0;
		if (key == KeyEvent.KEYCODE_DPAD_UP || key == KeyEvent.KEYCODE_DPAD_DOWN || key == KeyEvent.KEYCODE_O
	...
	public void showIntroScreen(Canvas canvas) {
		...
		canvas.drawText(s, (width - (s.length() * mTextPaint.getTextSize() / 2)) / 2,
	...
	public void drawPlayField(Canvas canvas) {
		...
		for (int a = 0; a < numStars; a++) {
			...
			// Draw starts
			canvas.drawRect(starsX[a], starsY[a], starsX[a] + 2, starsY[a] + 2, starsC[a]);
		}
		...
		if (firecnt != 0) {
			canvas.drawBitmap(fire[firecnt - 1], x + ((sxsize - sxfire) / 2), y + sysize, mBitmapPaint); // engine
																											// fire
		}
		firecnt++;
		if (firecnt > 2)
			...
			if (metx[n] >= 0) {
				...
				if (mety[n] > height - borderwidth - scoreheight) {
					...
					if (metr[n]) {
						...
						goff.drawBitmap(boom[bframes - metf[n]], metx[n] + (sxmet - sxbom) / 2, mety[n]
